...
require_once __DIR__ . '/../Comission.php';
...
class ComissionTest extends PHPUnit_Framework_TestCase {
    ...
    public function testValidateInt($a='a',$b=1,$c=1,$expected='error'){
        ...
        try {
			$this -> comission -> validateInt($a,$b,$c);
    ...
    public function testValidateRange($a=0,$b=0,$c=0,$expected='error'){
		echo __FUNCTION__,"(",$a,",",$b,",",$c,",",$expected,") --> ";
        try {
			$this -> comission -> validateRange($a,$b,$c);
			$this -> ValidateFunc(1,$expected);
        } catch(Exception $e) {
			$this -> ValidateFunc(2,$expected,$e);
        }
     ...
     */
	public function testNextDate($a=1,$b=1,$c=1,$expected="2014-10-26") {
		echo __FUNCTION__,"(",$a,",",$b,",",$c,",",$expected,") --> ";
		try {
			$this -> assertEquals($expected, $this -> comission -> calComission($a, $b, $c));
			echo "result(",$expected,")->OK", "\n";//達到預期效果，assert OK
			//$this -> assertFalse(false);
		} catch(Exception $e) {
			echo "result(",$this -> comission -> calComission($a, $b, $c),")->X!", "\n";//非預期效果，assert 有問題
